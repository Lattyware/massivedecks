 ...
 */
export const targetAll =
  (event: Event): Distributor =>
  (lobby, send) => {
...
export const targetOnly =
  (event: Event, ...targets: User.Id[]): Distributor =>
  (lobby, send) => {
    const rendered = JSON.stringify(event);
...
export const playerSpecificAddition =
  <T extends Event, U extends Partial<T>>(
    event: T,
...
export const targetAllAndPotentiallyClose =
  (event: Event, close: (userId: User.Id) => boolean): Distributor =>
  (lobby, send) => {
    const rendered = JSON.stringify(event);
    for (const id of Object.keys(lobby.users)) {
